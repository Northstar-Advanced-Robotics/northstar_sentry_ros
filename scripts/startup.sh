#!/bin/bash
# Boot entry point for the localization stack WITH rviz (localization.service).
# Runs on the HOST: waits for the desktop + cameras, then launches the container.
set -euo pipefail

source "$(dirname "${BASH_SOURCE[0]}")/_sentry_env.sh"

# rviz needs a display - wait for one and export DISPLAY/XAUTHORITY.
sentry_wait_for_x
...
exec docker run --rm "${SENTRY_DOCKER_ARGS[@]}" "$SENTRY_IMAGE" \
  bash -c './scripts/run.sh localization'
