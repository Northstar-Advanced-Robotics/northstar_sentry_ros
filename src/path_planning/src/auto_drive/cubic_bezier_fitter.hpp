...
public:
        ...
        float tol = 1e-6f)
    {
        ...
        for (int iter = 0; iter < maxIters; iter++)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                Eigen::Vector2f B = u * u * u * P0 + B1 * P1 + B2 * P2 + ti * ti * ti * P3;
                ...
                Eigen::Vector2f J[4] = {dB_da, dB_db, dB_dc, dB_dd};
        ...
        for (int iter = 0; iter < maxIters; iter++)
        {
            ...
            if (newError < currentError)
            {
                float relativeChange =
                    std::abs(currentError - newError) / std::max(1e-12f, currentError);
    ...
    // static std::vector<Eigen::Vector2f> PopulatePoints(std::vector<Eigen::Vector2i> pathInt,
    // float spacing)
    // {
    ...
    static std::vector<Eigen::Vector2f> PopulatePoints(
        const std::vector<Eigen::Vector2i>& pathInt,
        float spacing)
    {
        // 1. Convert to Float
        std::vector<Eigen::Vector2f> path;
    ...
    static std::vector<Eigen::Vector2f> PopulatePointsNew(
        const std::vector<Eigen::Vector2i>& pathInt)
    {
        const float POINTS_PER_DIST = 18.0f;  // in grid space
        const float POINTS_PER_RAD = 1.0f;

        if (pathInt.size() < 2) return {};

        std::vector<Eigen::Vector2f> path;
        path.reserve(pathInt.size());
        for (const auto& pi : pathInt) path.push_back(pi.cast<float>());
        ...
        for (size_t i = 0; i + 1 < path.size(); ++i)
        {
            ...
            int anglePoints =
                i == 0 ? 0 : std::abs(angle(path[i - 1], path[i], path[i + 1])) * POINTS_PER_RAD;
...
private:
    ...
    static Eigen::Vector2f BezierPoint(
        float t,
    ...
    static Eigen::Vector2f BezierDerivative(
        float t,
        const Eigen::Vector2f& P0,
        const Eigen::Vector2f& P1,
        const Eigen::Vector2f& P2,
    ...
    static Eigen::Vector2f BezierSecondDerivative(
        float t,
        const Eigen::Vector2f& P0,
        const Eigen::Vector2f& P1,
        const Eigen::Vector2f& P2,
    ...
    static float ProjectPointOntoBezier(
...
public:
    static int findFirstConcavityFlip(std::vector<Eigen::Vector2i> pts)
    {
        ...
        for (int i = 1; i < pts.size() - 1; i++)
        {
            ...
            if (sign == 0) continue;  // straight, ignore
