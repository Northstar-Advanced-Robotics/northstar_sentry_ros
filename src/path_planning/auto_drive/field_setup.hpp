...
public:
    ...
    // in meters, defined as pairs of bottom-left and top-right corners with the origin as the center of the field
    const std::vector<std::pair<Eigen::Vector2d, Eigen::Vector2d>> obstacleBounds;

    FieldSetup(float fieldRealSizeX, float fieldRealSizeY, float nodeSizeMeters, std::vector<std::pair<Eigen::Vector2d, Eigen::Vector2d>> obstacleBounds)
