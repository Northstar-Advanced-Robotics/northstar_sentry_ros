...
public:
    ...
    ThetaStar() : obstacleGrid(fieldSetup.gridSizeX * fieldSetup.gridSizeY, std::byte{0}),
    ...
    bool isNodeValidAndEmpty(Eigen::Vector2i point) {
        if (point.x() < 0 || point.x() >= fieldSetup.gridSizeX || point.y() < 0 || point.y() >= fieldSetup.gridSizeY) { return false; }
        return (getWorkingGrid(point.x(), point.y()) == std::byte{0});
    }

    void updateDynamicObstacles()
    {
        if (dynamicObstacles.empty()) { return; }

        copyObstaclesToWorkingGrid();

        int64_t currentTimeMS = std::chrono::steady_clock::now().time_since_epoch().count() / 1'000'000;
    ...
    std::vector<Eigen::Vector2i> FindPath(Eigen::Vector2i start, Eigen::Vector2i end)
    {
        ...
        while (!frontier.empty())
        {
            ...
            for(const auto& dir : dirs) {
                ...
                if (!isNodeValidAndEmpty(neighbor)) { continue; }
                ...
                if (cameFrom.find(current) != cameFrom.end()) {
                    ...
                    if (parent != current) {
                        if (lineOfSight(parent, neighbor)) {
                            ...
                            float newCost = costSoFar[parent] + (parent - neighbor).cast<float>().norm();
    ...
    {
        return Eigen::Vector2f((gridPos.x() / (float)fieldSetup.gridSizeX) * fieldSetup.fieldRealSizeX - fieldSetup.fieldRealSizeX / 2.0f, (gridPos.y() / (float)fieldSetup.gridSizeY) * fieldSetup.fieldRealSizeY - fieldSetup.fieldRealSizeY / 2.0f);
    }
...
private:
        ...
        for (int x = -nodeRadius; x <= nodeRadius; x++) {
            for (int y = -nodeRadius; y <= nodeRadius; y++) {
                if (x*x + y*y <= r2) {
                    ...
                    if (targetX >= 0 && targetX < fieldSetup.gridSizeX && targetY >= 0 && targetY < fieldSetup.gridSizeY) {
    ...
    void createObstacles() {
        for (const auto& bound : fieldSetup.obstacleBounds) {
            Eigen::Vector2i minGrid = ConvertWorldToGrid(Eigen::Vector2f(bound.first.x(), bound.first.y()));
            Eigen::Vector2i maxGrid = ConvertWorldToGrid(Eigen::Vector2f(bound.second.x(), bound.second.y()));
            ...
            for (int y = 0; y < fieldSetup.gridSizeY; y++)
            {
                if (getObstacleGrid(x, y) == std::byte{1})
                {
                    ...
                    for (int i = minX; i < maxX; i++)
                    {
                        ...
                        {
                            if (i >= 0 && i < fieldSetup.gridSizeX && j >= 0 && j < fieldSetup.gridSizeY) 
                            {
