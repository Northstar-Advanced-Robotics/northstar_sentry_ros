...
private:
        ...
        const std::string& cam_name)
    {
        ...
        if (data.count > 0)
        {
            ...
            {
                // Approximate area using the corners (0: top-left, 1: top-right, 2: bot-right, 3:
                // bot-left)
...
public:
    COVFilter() : Node("cov_filter")
    {
        ...
        odom_sub_ = this->create_subscription<nav_msgs::msg::Odometry>(
            "/odom/body_rig",
            10,
            std::bind(&COVFilter::odom_callback, this, _1));

        filtered_odom_pub_ =
            this->create_publisher<nav_msgs::msg::Odometry>("/odom/body_rig/filtered", 10);
        ...
        RCLCPP_INFO(
            this->get_logger(),
            "Covariance Filter Node Started. Listening to %zu cameras.",
            cameras.size());
    }
};
