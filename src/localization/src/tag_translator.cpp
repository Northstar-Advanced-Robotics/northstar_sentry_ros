...
private:
    ...
    void tag_callback(
        const isaac_ros_apriltag_interfaces::msg::AprilTagDetectionArray::SharedPtr isaac_msg)
    {
...
public:
    ...
    {
        sub_ =
            this->create_subscription<isaac_ros_apriltag_interfaces::msg::AprilTagDetectionArray>(
                input_topic,
                rclcpp::SensorDataQoS(),
                std::bind(&TagTranslator::tag_callback, this, _1));
...
int main(int argc, char** argv)
{
    ...
    auto trans_left = std::make_shared<TagTranslator>(
        "trans_left",
        "/left/detector/tags_nvidia",
        "/left/detector/tags");
    auto trans_back = std::make_shared<TagTranslator>(
        "trans_back",
        "/back/detector/tags_nvidia",
        "/back/detector/tags");
    auto trans_right = std::make_shared<TagTranslator>(
        "trans_right",
