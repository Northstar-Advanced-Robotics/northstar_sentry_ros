...
private:
    ...
    void request_robot_id()
    {
        if (robotid_msg.robot_id != src::uart::RobotIDMessage::RobotId::INVALID)
        {
            robotid_request_timer_->cancel();
            RCLCPP_INFO(
                this->get_logger(),
                "Robot ID acquired: %d",
                static_cast<int>(robotid_msg.robot_id));
...
public:
    ...
    {
        alive_pub_ =
            this->create_publisher<std_msgs::msg::Empty>("uart/heartbeat", rclcpp::SensorDataQoS());
        ...
        tf2_listener = std::make_unique<tf2_ros::TransformListener>(*tf2_buffer_);
    ...
    void start()
    {
        ...
        odom_from_mcb_timer_ =
            this->create_wall_timer(10ms, std::bind(&UartBridge::publish_odom, this));

        robotid_request_timer_ =
            this->create_wall_timer(500ms, std::bind(&UartBridge::request_robot_id, this));
    }
};
